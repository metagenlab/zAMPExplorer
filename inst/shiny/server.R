...
shinyServer(function(input, output, session) {
  ...
  # Observe button clicks and update reactive value
  observeEvent(input$show_reads_distribution_samples, { output_reads_qc_view("reads_distribution_samples") })
  observeEvent(input$show_reads_distribution_groups, { output_reads_qc_view("reads_distribution_groups") })
  observeEvent(input$show_number_of_reads, { output_reads_qc_view("number_of_reads") })
  observeEvent(input$show_rarefaction_curves, { output_reads_qc_view("rarefaction_curves") })
  ...
  output$download_plot_samples <- downloadHandler(
    ...
    content = function(file) {
      p <- ggplot(data.frame(Reads = sample_sums(physeq())), aes(x = Reads)) +
        geom_histogram(binwidth = 10000, fill = "blue", color = "black", alpha = 0.7) +
        labs(title = "Reads Distribution Across Samples", x = "Number of Reads", y = "Frequency") +
        theme_minimal()
...
#   })
  ...
  observeEvent(input$calculate_dominant_taxa, {
    ...
    tryCatch({
      ...
      p <- comp_barplot(physeq_dominant, tax_level = taxa_level, facet_by = input$group_column,
...
### upset_plot
  ...
  #
  #   # Ensure 'chao1' or any other metric is available in `tab`
  #   available_metrics <- colnames(tab)
  #
  #   # Extract sample data and add diversity metrics if they exist
  ...
  #
  #   # Update selectInput choices based on available metrics
  #   updateSelectInput(session, "alphaMetric", choices = available_metrics)
  ...
  output$alphaDiversityPlot <- renderPlotly({
    ...
    if (length(plots) == 1) {
      plots[[1]] %>%
        layout(dragmode = "zoom", margin = list(l = 120, r = 40, t = 40, b = 40), showlegend = TRUE) %>%
  ...
  observe({
    ...
    updateSelectInput(session, "groupingColumnBeta", choices = colnames(sample_data(current_physeq())))
    updateSelectInput(session, "shapeColumnBeta", choices = colnames(sample_data(current_physeq())))
  })
    ...
    get_transformed_pseq <- function(pseq, distanceMetric, normalizationMethod, taxRank) {
      if (distanceMetric == "aitchison" && normalizationMethod != "identity") {
    ...
    # Initialize reactive values for plot objects
    ...
    observeEvent(input$plotPCoA, {
      req(current_physeq(), input$distanceMetric, input$normalizationMethod, input$taxRank, input$groupingColumnBeta, input$shapeColumnBeta)

      # Access the actual phyloseq object
      pseq <- current_physeq()

      tryCatch({
        # Get transformed phyloseq object based on input
        ...
        # Plot with ord_plot
        plot <- ord_plot(ordination_res, color = input$groupingColumnBeta, shape = input$shapeColumnBeta, plot_taxa = 1:5) +
          stat_ellipse(aes_string(fill = input$groupingColumnBeta, color = input$groupingColumnBeta), linetype = 1, segments = 10, lwd = 1.2, alpha = 0.25, level = 0.6) +
    ...
    observeEvent(input$plotPCA, {
      req(current_physeq(), input$normalizationMethod, input$taxRank, input$groupingColumnBeta, input$shapeColumnBeta)

      pseq <- current_physeq()
      tryCatch({
        # Transform data and calculate PCA
        ordination_res <- pseq %>%
  ...
  observeEvent(input$Statistics, {
    req(current_physeq(), input$distanceMetric, input$normalizationMethod, input$taxRank, input$groupingColumnBeta)
    ...
    tryCatch({
      ...
      # Apply relative abundance transform
      tse <- transformSamples(tse, method = "relabundance")

      # Extract transformed data for PERMANOVA
      assay_data <- t(assay(tse, "relabundance"))

      # PERMANOVA Analysis
      set.seed(12346)
      # Updated to use input$groupingColumnBeta directly
      permanova_res <- vegan::adonis2(
        assay_data ~ get(input$groupingColumnBeta), # Directly use input$groupingColumnBeta
        data = colData(tse),
      ...
      # Render PERMANOVA results
      output$permanovaTable <- DT::renderDataTable({
        DT::datatable(as.data.frame(permanova_res), options = list(pageLength = 5))
      })

      # Download handler for PERMANOVA results
      output$download_permanovaTable <- downloadHandler(
        filename = function() {
          paste0("permanova_", input$distanceMetric, "_", Sys.Date(), ".csv")
        },
        content = function(file) {
          write.csv(as.data.frame(permanova_res), file)
        }
      )

      # Dispersion Test Analysis
      dist_matrix <- vegan::vegdist(assay_data, method = input$distanceMetric)
      betadisp_res <- vegan::betadisper(dist_matrix, colData(tse)[[input$groupingColumnBeta]])
      dispersion_anova <- anova(betadisp_res)

      # Render Dispersion Test results
      output$betadisperTable <- DT::renderDataTable({
        DT::datatable(as.data.frame(dispersion_anova), options = list(pageLength = 5))
      })

      # Download handler for Dispersion Test results
      ...
      boxplot_data <- data.frame(
        distance = betadisp_res$distances,
        groupingColumnBeta = colData(tse)[[input$groupingColumnBeta]]
      )

      p <- ggplot(boxplot_data, aes(x = groupingColumnBeta, y = distance)) +
        geom_boxplot() +
        theme_light() +
        labs(title = "Dispersion to Centroid by Group", x = "Group", y = "Distance to Centroid")
      ...
      # Display error in case something goes wrong
      showModal(modalDialog(
        title = "Error in Statistical Analysis",
        e$message,
        easyClose = TRUE,
        footer = modalButton("Close")
      ))
