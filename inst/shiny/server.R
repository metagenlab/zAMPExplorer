...
### upset_plot
  ...
  observeEvent(input$plotNMDS, {
    ...
    tryCatch({
      ...
      plot <- ord_plot(
        ordination_res,
        color = input$groupingColumnBeta,
        shape = input$shapeColumnBeta,
        plot_taxa = 1:5
      ) +
        stat_ellipse(
          aes_string(fill = input$groupingColumnBeta, color = input$groupingColumnBeta),
          linetype = 1,
          segments = 10,
          lwd = 1.2,
          alpha = 0.25,
        ...
        ggside::geom_xsideboxplot(
          aes_string(fill = input$groupingColumnBeta, y = input$groupingColumnBeta),
          orientation = "y"
        ) +
        ggside::geom_ysideboxplot(
          aes_string(fill = input$groupingColumnBeta, x = input$groupingColumnBeta),
          orientation = "x"
        ) +
        ggside::scale_xsidey_discrete(labels = NULL) +
        ggside::scale_ysidex_discrete(labels = NULL) +
 ...
 ########### statistics tab 
  ...
  observe({
    req(current_physeq()) # Ensure current_physeq() is available
    pseq <- current_physeq()
    
    # Extract metadata column names
    validate(need(!is.null(sample_data(pseq)), "No metadata found in the phyloseq object.")) 
    
  })
  
  observeEvent(input$calculatePERMANOVA, {
    req(current_physeq(), input$groupingColumnBeta, input$distanceMetric)
    pseq <- current_physeq()
    
    # Transform the phyloseq object to compositional data
    pseq_rel <- microbiome::transform(pseq, "compositional")
    otu <- abundances(pseq_rel)
    meta <- meta(pseq_rel)
    
    # Validate grouping column and strata
    validate(need(input$groupingColumnBeta %in% colnames(sample_data(pseq)), "Grouping column not found in metadata."))
    strata_var <- if (input$strata != "None") input$strata else NULL
    
    tryCatch({
      # Perform PERMANOVA
      permanova <- adonis2(
        formula = as.formula(paste("t(otu) ~", input$groupingColumnBeta)),
        data = meta,
        permutations = 99,
        method = input$distanceMetric,
        strata = if (!is.null(strata_var)) meta[[strata_var]] else NULL
      )
      ...
      # Extract coefficients for the top taxa
      coef <- coefficients(permanova)[[input$groupingColumnBeta]]
      top_coef <- coef[rev(order(abs(coef)))[1:20]]
      
      # Render the top taxa barplot
      output$topTaxaPlot <- renderPlot({
        barplot(sort(top_coef), horiz = TRUE, las = 1, main = "Top taxa separating groups")
      })
      
      # Enable download of top taxa coefficients
      output$downloadTopTaxa <- downloadHandler(
        filename = function() {
          paste0("top_taxa_", Sys.Date(), ".csv")
        },
        content = function(file) {
          write.csv(data.frame(Taxa = names(top_coef), Coefficients = top_coef), file, row.names = FALSE)
        }
      )
    }, error = function(e) {
      showModal(modalDialog(
  ...
  observeEvent(input$calculateDispersionTest, {
    req(current_physeq(), input$distanceMetric, input$groupingColumnBeta)
    
    pseq <- current_physeq()
    validate(need(!is.null(pseq), "Phyloseq object is missing."))
    validate(need(input$groupingColumnBeta %in% colnames(sample_data(pseq)), "Selected grouping column not found."))
    ...
    tryCatch({
      ...
      grouping_var <- sample_data(pseq_relabund)[[input$groupingColumnBeta]]
  ...
  observe({
    shinyDirChoose(input, "select_folder", roots = roots, session = session, allowDirCreate = TRUE) # Enable directory creation
  })
