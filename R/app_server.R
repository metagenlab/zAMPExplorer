...
#' @importFrom plotly plotlyOutput renderPlotly ggplotly config subplot as_widget
...
app_server <- function(input, output, session) {
  ...
  if (!requireNamespace("microViz", quietly = TRUE)) {
    stop("The package 'microViz' is required for this functionality. Please install it using utils::install.packages('microViz').")
  }
  ...
  rarefaction_plot <- reactive({
    ...
    prare <- MicrobiotaProcess::ggrarecurve(physeq,
  ...
  observeEvent(input$calculate_dominant_taxa, {
    ...
    tryCatch({
      ...
      p <- microViz::comp_barplot(physeq_dominant, tax_level = taxa_level, facet_by = input$group_column,
        ...
        ggplot2::coord_flip() +
        ggplot2::labs(title = paste("Dominant Taxa Composition in each Sample"))
  ...
  output$download_alphaDiversityPlot <- downloadHandler(
    ...
    content = function(file) {
      if (input$alpha_filetype == "html") {
        ...
        if (length(plots) > 1) {
          p <- gridExtra::marrangeGrob(plots, ncol = 2, nrow = ceiling(length(plots) / 2))
        } else {
