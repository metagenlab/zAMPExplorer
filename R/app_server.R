...
#' @import shiny
#' @importFrom ggplot2 ggplot aes geom_bar geom_histogram geom_boxplot theme arrow unit labs element_text margin ggtitle scale_y_log10 ggsave geom_jitter scale_color_manual theme_bw
#' @importFrom utils combn write.csv write.table
#' @importFrom DT datatable DTOutput renderDT
#' @importFrom plotly plotlyOutput renderPlotly ggplotly config subplot as_widget kaleido
...
#' @importFrom phyloseq phyloseq distance otu_table sample_data rank_names sample_variables taxa_sums nsamples tax_table prune_samples sample_sums sample_names sample_data phy_tree ntaxa prune_taxa
#' @importFrom dplyr filter mutate select
#' @importFrom tidyr pivot_longer pivot_wider
#' @importFrom vegan capscale adonis vegdist scores
#' @importFrom writexl write_xlsx
#' @importFrom htmlwidgets saveWidget
#' @importFrom microbiome transform aggregate_taxa core core_members plot_composition
...
app_server <- function(input, output, session) {
  ...
  physeq <- reactive({
    ...
    phyloseq::sample_data(pseq) <-
      phyloseq::sample_data(pseq)[,!(names(phyloseq::sample_data(pseq)) %in% drop)]
  ...
  observe({
    if (!is.null(physeq())) {
      message("Phyloseq object has been uploaded and processed.\n")
    } else {
      message("No phyloseq object uploaded yet.\n")
    }
  })
  ...
  observeEvent(physeq(), {
    ...
    showNotification("Phyloseq object successfully uploaded and processed.",
                     type = "message")
    ...
    updateSelectInput(session,
                      "label_tips",
                      "Label Tips By (taxonomy):",
                      choices = rank_names(physeq()))
    updateSelectInput(session,
                      "color_by",
                      "Choose Color By (metadata):",
                      choices = sample_variables(physeq()))
    updateSelectInput(session,
                      "shape_by",
                      "Choose Shape By (taxonomy):",
                      choices = rank_names(physeq()))
    ...
    updateSelectInput(session, "TaxRankcomp", choices = rank_names(physeq()))
    ...
    function(render_plot,
      ...
      downloadHandler(
        ...
        content = function(file) {
          ...
          htmlwidgets::saveWidget(plotly::as_widget(p), tempfile)
  ...
  output$dynamic_tables <- renderUI({
    ...
    if (output_view() == "metadata") {
      ...
      fluidRow(
        box(
          title = "Abundance/Taxonomy Table",
          width = 12,
          status = "primary",
          ...
          selectInput(
            "filetype",
            "Choose file type:",
            choices = c("csv", "xlsx", "tsv")
          ),
          downloadButton(
            "download_combined_table",
            "Download Abundance/Taxonomy Table"
          )
        )
      )
  ...
  combined_table <- reactive({
    ...
    combined <-
      merge(otu_table, taxonomy_table, by = "row.names", all = TRUE)
  ...
  output$combined_table <- DT::renderDT({
    ...
    validate(need(
      !is.null(phyloseq::otu_table(physeq())),
  ...
  output$download_combined_table <- downloadHandler(
    filename = function() {
      paste0("abundance_taxonomy_table_",
             Sys.Date(),
             ".",
             input$filetype)
    },
  ...
  output$summary_statistics <- DT::renderDT({
    ...
    validate(
      ...
      need(
        !is.null(phyloseq::tax_table(physeq())),
        "Error: Taxonomy table is missing."
      )
    )
    ...
    num_phylum <-
      length(unique(phyloseq::tax_table(physeq())[, "Phylum"]))
    num_family <-
      length(unique(phyloseq::tax_table(physeq())[, "Family"]))
    num_genus <-
      length(unique(phyloseq::tax_table(physeq())[, "Genus"]))
    num_species <-
      length(unique(phyloseq::tax_table(physeq())[, "Species"]))
  ...
  observeEvent(input$explanation_button_reads_qc, {
    showModal(
      modalDialog(
        ...
        p(
          "This section provides quality control analysis for sequencing reads."
        ),
        p(
          "You can analyze the distribution of reads across samples or groups, view the number of reads per sample, or plot rarefaction curves to assess sequencing depth sufficiency."
        ),
        easyClose = TRUE,
    ...
    if (output_reads_qc_view() == "reads_distribution_groups") {
      fluidRow(column(
        ...
        div(
          ...
          plotlyOutput(
            "reads_distribution_boxplot_groups",
            height = "600px",
            width = "90%"
          )
        )
      ...
      fluidRow(
        column(
          ...
          numericInput(
            "reads_threshold",
            "Reads Threshold for Sample Filtration",
            value = 1000,
            min = 0
          ),
          selectInput(
          ...
          downloadButton(
            "download_filtered_physeq",
            "Download Filtered Phyloseq"
          )
        )
      )
  ...
  downloadOptions <- function(prefix) {
    list(
      selectInput(
        paste0("plot_filetype_", prefix),
        "Select File Type",
        choices = c("png", "pdf", "svg")
      ),
      numericInput(
        paste0("plot_width_", prefix),
        "Plot Width (inches)",
        value = 8,
        min = 4
      ),
      numericInput(
        paste0("plot_height_", prefix),
        "Plot Height (inches)",
        value = 6,
        min = 4
      ),
  ...
  output$reads_distribution_plot_samples <- renderPlotly({
    req(physeq())
    reads_per_sample <- phyloseq::sample_sums(physeq())
    p <-
      ggplot2::ggplot(data.frame(Reads = reads_per_sample), ggplot2::aes(x = Reads)) +
      ...
      ggplot2::labs(title = "Reads Distribution Across Samples", x = "Number of Reads", y = "Frequency") +
  ...
  output$download_plot_samples <- downloadHandler(
    filename = function() {
      paste0("reads_distribution_samples_",
             Sys.Date(),
             ".",
             input$plot_filetype_samples)
    },
    content = function(file) {
      p <-
        ggplot2::ggplot(data.frame(Reads = sample_sums(physeq())), ggplot2::aes(x = Reads)) +
        ggplot2::geom_histogram(
          binwidth = 10000,
          fill = "blue",
          color = "black",
      ...
      ggplot2::ggsave(
        filename = file,
        plot = p,
        device = input$plot_filetype_samples,
        width = input$plot_width_samples,
        height = input$plot_height_samples
      )
    }
  )
  ...
  output$reads_distribution_boxplot_groups <- renderPlotly({
    ...
    validate(
      need(
        input$group_column != "",
        "Please select a grouping column to generate the plot."
      )
    )
    ...
    p <-
      ggplot2::ggplot(reads_df, ggplot2::aes(x = Group, y = Reads, fill = Group)) +
      ggplot2::geom_boxplot(alpha = 0.7, outlier.color = "red") +
      ggplot2::geom_jitter(
        ggplot2::aes(text = paste(
          "Sample:", Sample, "<br>Reads:", Reads
        )),
        color = "black",
        size = 1.5,
        width = 0.2,
      ...
      ggplot2::labs(title = "Reads Distribution per Sample Group",
                    x = "Group",
                    y = "Number of Reads") +
      ggplot2::theme_minimal() +
      ggplot2::theme(
  ...
  output$download_boxplot_groups <- downloadHandler(
    filename = function() {
      paste0("reads_distribution_boxplot_",
             Sys.Date(),
             ".",
             input$plot_filetype_groups)
    },
    content = function(file) {
      ...
      p <-
        ggplot2::ggplot(reads_df, aes(x = Group, y = Reads, fill = Group)) +
        ggplot2::geom_boxplot(alpha = 0.7, outlier.color = "red") +
        ggplot2::geom_jitter(
          ggplot2::aes(text = paste(
            "Sample:", Sample, "<br>Reads:", Reads
        ...
        ggplot2::labs(title = "Reads Distribution per Sample Group", x = "Group", y = "Number of Reads") +
        ggplot2::theme_minimal() +
  ...
  output$number_of_reads_table <- DT::renderDT({
    ...
    reads_df <-
      data.frame(Sample = sample_names(current_physeq()),
                 Reads = sample_sums(current_physeq()))
  ...
  observeEvent(input$filter_samples, {
    ...
    if (!is.null(input$sample_name_filter) &&
        length(input$sample_name_filter) > 0) {
      filtered <-
        phyloseq::prune_samples(!sample_names(filtered) %in% input$sample_name_filter,
                                filtered)
      filtered <-
        phyloseq::prune_taxa(taxa_sums(filtered) > 0, filtered)
    }
    ...
    output$filtered_samples_table <- DT::renderDT({
      filtered_samples_df <-
        data.frame(Sample = sample_names(current_physeq()),
                   Reads = sample_sums(current_physeq()))
      DT::datatable(filtered_samples_df,
                    options = list(pageLength = 10, scrollX = TRUE))
    })
  ...
  output$reads_per_sample_plot <- renderPlotly({
    ...
    samples_df <-
      data.frame(Sample = sample_names(current_physeq()), Reads = reads_per_sample)

    p <- ggplot2::ggplot(samples_df, ggplot2::aes(x = Sample, y = Reads)) +
      ggplot2::geom_bar(stat = "identity", fill = "steelblue") +
      ggplot2::labs(title = "Total Number of Reads per Sample", x = "Sample", y = "Total Reads") +
      ggplot2::theme_minimal() +
  ...
  output$download_reads_plot <- downloadHandler(
    filename = function() {
      paste0("reads_per_sample_plot_",
             Sys.Date(),
             ".",
             input$plot_filetype_reads)
    },
    content = function(file) {
      reads_per_sample <- sample_sums(current_physeq())
      samples_df <-
      ...
      ggplot2::ggsave(
        filename = file,
        plot = p,
        device = input$plot_filetype_reads,
        width = input$plot_width_reads,
        height = input$plot_height_reads
      )
    }
  )
  ...
  rarefaction_plot <- reactive({
    ...
    rareres <-
      rarefaction_data() # Use the reactive rarefaction data

    # Create the ggplot2 rarefaction plot
    prare <- MicrobiotaProcess::ggrarecurve(
      physeq,
      indexNames = c("Observe", "Chao1", "ACE"),
      shadow = TRUE,
      factorNames = input$group_column,
    ) +
      ggplot2::theme(legend.spacing.y = unit(0.02, "cm"),
    ...
    if (output_taxa_overview_view() == "taxa_prevalence_samples") {
      fluidRow(
        box(
          ...
          downloadButton(
            "download_taxa_prevalence_samples",
            "Download Taxa Prevalence Data"
          )
        )
      )
      ...
      fluidRow(
        box(
          title = "Taxa Prevalence Across Groups",
          width = 12,
          status = "primary",
          selectInput(
            "taxa_level_groups",
          ...
          downloadButton(
            "download_taxa_prevalence_groups",
            "Download Taxa Prevalence Data"
          )
        )
      )
      ...
      fluidRow(
        box(
          title = "Dominant Taxa Per Sample",
          width = 12,
          status = "primary",
          selectInput(
            "dominant_taxa_level_samples",
            "Select Taxonomic Level",
            choices = rank_names(physeq())
          ),
          selectInput(
          ...
          numericInput(
            "Abundance_threshold",
            "Set Abundance Threshold (0 to 1)",
            value = 0.001,
            min = 0,
            max = 1,
            step = 0.001
          ...
          numericInput(
            "n_max_dominant",
            "Set Maximum Number of Dominant Taxa",
            value = 10,
            min = 3
          ),
          ...
          plotOutput(
            "dominant_taxa_barplot",
            height = "600px",
            width = "100%"
          ),
          selectInput(
            "plot_filetype_groups",
            "Select File Type",
            choices = c("png", "pdf", "svg")
          ),
          numericInput(
          ...
          downloadButton(
            "download_dominant_taxa_plot",
            "Download Dominant Taxa Plot"
          )
        )
      )
      ...
      fluidRow(
        box(
          title = "Prevalence vs Abundance Plot",
          width = 12,
          status = "primary",
          plotlyOutput(
          ...
          downloadButton(
            "download_prevalence_abundance_plot",
            "Download Prevalence vs Abundance Plot"
          )
        )
      )
      ...
      fluidRow(
        box(
          ...
          downloadButton(
            "download_core_microbiome_plot",
            "Download Core Microbiome Plot"
          )
        )
      )
    }
  ...
  observeEvent(input$explain_taxa_prevalence_samples, {
    showModal(
      modalDialog(
        ...
        p(
          "This section allows you to explore the prevalence of taxa across all samples at different taxonomic levels."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  observeEvent(input$explain_taxa_prevalence_groups, {
    showModal(
      modalDialog(
        ...
        p(
          "This section allows you to compare the prevalence of taxa across different groups at different taxonomic levels."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  observeEvent(input$explain_dominant_taxa_samples, {
    showModal(
      modalDialog(
        ...
        p(
          "This section allows you to determine the dominant taxa within each sample at different taxonomic levels."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  output$taxa_prevalence_table_samples <- DT::renderDT({
    ...
    prevalence_df <-
      apply(as.data.frame(otu_table(physeq_agg)), 1, function(x)
        sum(x > 0) / nsamples(physeq_agg) * 100)
    prevalence_df <-
      round(prevalence_df, 3) # Round to 3 decimal places

    absolute_abundance <- rowSums(otu_table(physeq_agg))

    prevalence_table <-
      data.frame(
        Species = phyloseq::tax_table(physeq_agg)[, taxa_level],
  ...
  output$taxa_prevalence_table_groups <- DT::renderDT({
    ...
    groups <-
      as.factor(phyloseq::sample_data(physeq_agg)[[group_column]])
    ...
    prevalence_list <- lapply(levels(groups), function(group) {
      ...
      apply(otu_table_agg[, group_samples], 1, function(x)
        round(sum(x > 0) / length(group_samples) * 100, 2))
    })
    ...
    prevalence_table <-
      data.frame(Species = phyloseq::tax_table(physeq_agg)[, taxa_level], prevalence_df)
  ...
  prevalence_abundance_plot <- reactive({
    ...
    ps_rel <-
      microbiome::transform(current_physeq(), "compositional")
    ...
    p <-
      ...
      ggplot2::scale_y_log10() + # Log scale for abundance
      ggplot2::theme_minimal() +
      ggplot2::labs(title = "Prevalence vs Abundance", x = "Prevalence (%)", y = "Abundance") +
  ...
  output$download_prevalence_abundance_plot <- downloadHandler(
    filename = function() {
      paste0("prevalence_vs_abundance_",
             Sys.Date(),
             ".",
             input$plot_filetype_groups) # Dynamic filename based on date and file type
    },
    content = function(file) {
      # Reuse the ggplot object stored in the reactive expression
      ggplot2::ggsave(
        file,
        plot = prevalence_abundance_plot(),
        device = input$plot_filetype_groups,
        # Dynamic file type
        width = input$plot_width_groups,
        # Dynamic width
  ...
  observeEvent(input$explain_prevalence_abundance_plot, {
    showModal(
      modalDialog(
        ...
        p(
          "This plot compares the prevalence of each Genus (percentage of samples in which the Genus is present) with its total abundance across all samples. Points are colored by Phylum."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  generate_upset_plot <-
    function(physeq,
      ...
      physeq_core <-
        core(physeq, detection = detection_threshold, prevalence = prevalence_threshold)
      ...
      if (any(is.na(phyloseq::sample_data(physeq_core)[[group_var]]))) {
        stop(
          "The selected grouping column contains missing values. Please choose a different column or remove missing values."
        )
      }

      # Use get_upset to generate the binary matrix for upset plot
      upset_data <-
        MicrobiotaProcess::get_upset(physeq_core, factorNames = group_var)

      # Create the upset plot using sample groupings
      upset_plot <- UpSetR::upset(
        upset_data,
        sets = unique(as.vector(
          phyloseq::sample_data(physeq_core)[[group_var]]
        )),
        # Grouping by sample data
        sets.bar.color = "#56B4E9",
        keep.order = TRUE,
        # Keep the order of the sets
        order.by = "freq",
  ...
  output$upset_plot <- renderPlot({
    req(
      current_physeq(),
      input$detection_threshold_upset,
      input$prevalence_threshold_upset,
      input$group_column
    )
  ...
  output$download_upset_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (input$plot_filetype_upset == "png") {
        png(
          file,
          width = input$plot_width_upset,
          height = input$plot_height_upset,
          units = "in",
          res = 300
        )
  ...
  observeEvent(input$explain_upset_plot, {
    showModal(
      modalDialog(
        ...
        p(
          "This Upset plot shows the intersections of core taxa across samples."
        ),
        p(
          "It helps to identify shared or unique taxa across multiple groups or conditions."
        ),
        p(
          "Core taxa are defined based on user-adjustable detection and prevalence thresholds."
        ),
        tags$ul(
          tags$li(
            "**Set Size (left panel)**: The horizontal bars on the left represent the total number of taxa found in each group."
          ),
          tags$li(
            "**Intersection Size (top panel)**: The vertical bars at the top represent the number of taxa shared between the selected groups or unique to a specific group. Each dot and line combination below the bars shows the groups that are being compared in that intersection. For instance, a bar with a single dot below it indicates taxa unique to one group, whereas bars with lines connecting multiple dots indicate shared taxa across multiple groups."
          ),
          tags$li(
            "**Interpretation**: You can use this plot to see which taxa are unique to certain conditions (groups) or common to multiple groups. The higher the intersection size, the more core taxa are shared among the connected groups."
          )
        ),
        p(
          "Adjust the detection and prevalence thresholds to fine-tune the definition of core taxa based on your analysis needs."
        ),
        p(
          "Empty intersections in an UpSet plot can occur when certain combinations of groups (sets) have no overlapping/shared taxa. This means that, for the given threshold values of detection and prevalence, there are no taxa shared among those groups, so the intersection is effectively empty."
        ),
        p(
          "You can prevent empty intersections from being displayed in the UpSet plot by using the 'empty.intersections' = 'off' parameter in the server code when creating the plot."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  observeEvent(input$show_venn_diagram_plot, {
    output$taxa_overview_content <- renderUI({
      fluidRow(
        box(
          title = "Venn Diagram - Unique and Shared Taxa",
          width = 12,
          status = "primary",
          selectInput(
            "taxonomy_level",
            "Select Taxonomy Level",
            choices = rank_names(current_physeq())
          ),
          selectInput(
            "grouping_column_venn",
            "Select Grouping Column",
            choices = colnames(phyloseq::sample_data(current_physeq()))
          ),
          numericInput(
            "detection_threshold_venn",
            "Detection Threshold",
            value = 0.001,
            min = 0,
            max = 1,
          ...
          downloadButton(
            "download_shared_pairwise_taxa",
            "Download Pairwise Shared Taxa (TSV)"
          ),
          downloadButton(
            "download_shared_all_taxa",
            "Download Shared Across All Groups (TSV)"
          ),
          selectInput(
            "plot_filetype_venn",
            "Select File Type",
  ...
  group_otus_reactive <- reactive({
    req(
      ...
      input$detection_threshold_venn,
      input$prevalence_threshold_venn,
      input$grouping_column_venn
    )
    ...
    if (any(is.na(phyloseq::sample_data(current_physeq())[[input$grouping_column_venn]]))) {
      stop(
        "The selected grouping column contains missing values. Please choose a different column or clean the data."
      )
    }

    # Get the higher taxonomy level
    higher_taxonomy_level <-
      get_higher_taxonomy(current_physeq(), input$taxonomy_level)

    # Aggregate physeq data at the selected taxonomy level
    physeq_agg <-
      microbiome::aggregate_taxa(current_physeq(), input$taxonomy_level)
    ...
    physeq_core <-
      core(
        physeq_agg,
        detection = input$detection_threshold_venn,
        prevalence = input$prevalence_threshold_venn
      )
    ...
    group_otus <- lapply(sample_groups, function(group) {
      group_samples <-
        rownames(meta_data[meta_data[[input$grouping_column_venn]] == group,])
      otus_in_group <-
        rownames(otu_data[, group_samples][rowSums(otu_data[, group_samples]) > 0,])

      # Append the higher taxonomy level to the OTU names
      otus_with_higher_tax <- sapply(otus_in_group, function(otu) {
        higher_tax <-
          as.character(tax_table_data[otu, higher_taxonomy_level])
        otu_name <-
          as.character(tax_table_data[otu, input$taxonomy_level])
  ...
  output$download_venn_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      ggplot2::ggsave(
        file,
        plot = venn_plot,
        device = input$plot_filetype_venn,
  ...
  output$download_unique_taxa <- downloadHandler(
    ...
    content = function(file) {
      ...
      unique_taxa <- lapply(names(group_otus), function(group) {
        ...
        unique_to_group <-
          setdiff(group_otus[[group]], Reduce(union, group_otus[other_groups]))
      ...
      if (is.null(unique_taxa_df) || nrow(unique_taxa_df) == 0) {
        write.table(
          data.frame(Group = character(), Taxa = character()),
          file,
          sep = "\t",
          row.names = FALSE,
          col.names = TRUE,
  ...
  output$download_shared_pairwise_taxa <- downloadHandler(
    ...
    content = function(file) {
      ...
      group_combinations <-
        combn(names(group_otus), 2, simplify = FALSE)
      shared_taxa_list <-
        lapply(group_combinations, function(groups) {
          shared_otus <-
            intersect(group_otus[[groups[1]]], group_otus[[groups[2]]])
          if (length(shared_otus) > 0) {
            return(data.frame(
              Group1 = groups[1],
              Group2 = groups[2],
              Shared_Taxa = shared_otus
            ))
          } else {
            return(NULL)
          }
  ...
  observeEvent(input$explain_venn_diagram_plot, {
    showModal(
      modalDialog(
        ...
        p(
          "This Venn diagram displays the unique and shared taxa across multiple sample groups."
        ),
        p(
          "The analysis is performed at the taxonomy level selected by the user (e.g., Genus, Species). Additionally, core taxa are filtered based on the detection and prevalence thresholds provided."
        ),
        ...
        p(
          "This identifies the taxa shared between pairs of groups. Use the 'Download Pairwise Shared Taxa' button to obtain this data. It's valuable for understanding the common microbiota between two specific groups and can help in comparative studies."
        ),
        h4("Shared Taxa Across All Groups"),
        p(
          "These taxa are shared across all sample groups. Use the 'Download Shared Across All Groups' button to download this data. Identifying the core microbiota present in all groups can be helpful for finding broadly conserved organisms across different conditions or environments."
        ),
        h4("Venn Diagram Visualization"),
        p(
          "The Venn diagram provides a visual representation of the shared and unique taxa between the sample groups. Each section of the diagram corresponds to different sets of shared taxa between the groups. The overlap between two or more groups shows the taxa that are shared between them."
        ),
        p(
          "This analysis is particularly useful when comparing microbial communities from different environments, treatment groups, or conditions. By visualizing and downloading these insights, researchers can easily identify both common and unique taxa among their samples."
        ),
        easyClose = TRUE,
        footer = modalButton("Close")
      )
  ...
  compositional_barplot_reactive <- reactive({
    req(current_physeq(), input$TaxRankcomp, input$detectionThreshold, input$prevalenceThreshold)

    # Get the current phyloseq object
    pseq <- current_physeq()

    # Ensure taxa are present
    pseq <- phyloseq::prune_taxa(taxa_sums(pseq) > 0, pseq)

    # Merge rare taxa
    pseq.fam <- microbiome::aggregate_rare(
      pseq,
      level = input$TaxRankcomp,
      detection = input$detectionThreshold / 100,  # Convert percentage to proportion
      prevalence = input$prevalenceThreshold / 100  # Convert percentage to proportion
    )
    pseq.famrel <- microbiome::transform(pseq.fam, "compositional")

    # Dynamically generate colors for the number of taxa
    taxa_count <- ntaxa(pseq.famrel)  # Number of taxa
    color_palette <- colorRampPalette(brewer.pal(12, "Paired"))(taxa_count)  # Generate enough colors

    # Create the bar plot
    p.famrel <- microbiome::plot_composition(
      pseq.famrel,
      sample.sort = NULL,
      otu.sort = NULL,
      x.label = "Samples",
      ...
      ggplot2::labs(
        ...
        subtitle = paste("Taxonomic rank:", input$TaxRankcomp)
      ) +
      ggplot2::scale_fill_manual("Taxa", values = color_palette)  # Use custom color palette
  ...
  observe({
    req(current_physeq())
    ranks <- rank_names(current_physeq())
    updateSelectInput(session, "TaxRankcomp", choices = ranks, selected = "Genus")
  })

  output$compositionalBarplot <- renderPlotly({
    ggplotly(compositional_barplot_reactive())
  })
  ...
  output$download_compositionalBarplot <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (input$filetype == "html") {
        ...
        # Save as an image using kaleido
        plotly::kaleido(p, file = file, width = input$plot_width, height = input$plot_height, format = input$filetype)
      }
    }
  )
  ...
  observeEvent(input$show_compositional_explanation, {
    showModal(
      modalDialog(
        ...
        HTML(
          "<p>The Compositional Barplot visualizes the distribution of different taxa across samples, organized by selected taxonomic ranks.</p>
  ...
  observe({
    ...
    if (length(available_metrics) == 0) {
      showNotification("No diversity metrics available. Please check your data.",
                       type = "error")
      return()
    }
    ...
    phyloseq::sample_data(pseq) <-
      phyloseq::sample_data(meta) # Update sample data in phyloseq
  ...
  observeEvent(input$plotAlpha, {
    ...
    for (metric in input$alphaMetric) {
      if (metric %in% colnames(meta)) {
        # Check if metric exists in meta
        p <-
          ggplot2::ggplot(
            meta,
            ggplot2::aes_string(
              x = input$alphaGroupingColumn,
              y = metric,
              color = input$alphaGroupingColumn
            )
          ) +
  ...
  output$download_alphaDiversityPlot <- downloadHandler(
    ...
    content = function(file) {
      if (input$alpha_filetype == "html") {
        # Save Plotly as HTML
        plots <-
          plotly_plots() # Ensure this function returns valid Plotly objects
        p <-
          do.call(subplot, c(plots, list(
            nrows = ceiling(length(plots) / 2),
            shareX = TRUE,
            shareY = FALSE
          )))
        ...
        plots <-
          ggplot_plots() # Ensure this function returns valid ggplot objects

        # Check if `plots` is a list of ggplot objects
        if (length(plots) > 1) {
          p <-
            gridExtra::marrangeGrob(plots, ncol = 2, nrow = ceiling(length(plots) / 2))
        } else {
        ...
        ggplot2::ggsave(
          ...
          units = "in",
          device = input$alpha_filetype,
          # Ensure this matches supported devices
  ...
  observeEvent(input$showStats, {
    ...
    for (metric in input$alphaMetric) {
      ...
      p_values <- sapply(comparisons, function(comp) {
        ...
        group2 <-
          meta[meta[[input$alphaGroupingColumn]] == comp[2], metric]
      ...
      adj_p_values <-
        p.adjust(p_values, method = "BH", n = sum(!is.na(p_values)))
      ...
      stats_df <- data.frame(
        ...
        Adjusted_P_value = round(adj_p_values, 4),
        # Round adjusted p-values to 4 decimal places
  ...
  observeEvent(input$show_alpha_explanation, {
    showModal(
      modalDialog(
        ...
        HTML(
          "<p><strong>Alpha Diversity</strong> is a measure of the diversity within a single sample or environment.
  ...
  observe({
    ...
    updateSelectInput(session,
                      "groupingColumnBeta_plotting",
                      choices = colnames(phyloseq::sample_data(pseq)))
    updateSelectInput(session,
                      "groupingColumnBeta_statistics",
                      choices = colnames(phyloseq::sample_data(pseq)))
    updateSelectInput(session,
                      "shapeColumnBeta",
                      choices = colnames(phyloseq::sample_data(pseq)))
  ...
  get_transformed_pseq <-
    function(pseq,
             distanceMetric,
             normalizationMethod,
             taxRank) {
      if (distanceMetric == "aitchison" &&
          normalizationMethod != "identity") {
        stop(
          "'Aitchison' distance requires count data. Please select 'identity' as the normalization method or choose another distance metric."
        )
      }

      if (distanceMetric == "jaccard") {
        pseq_transformed <- pseq %>%
          microbiome::transform(normalizationMethod) %>%  # Normalize to relative abundance
          microbiome::aggregate_taxa(level = taxRank)%>%  # Aggregate at the desired rank
          phyloseq::distance(dist = "jaccard", binary = TRUE)
      } else if (distanceMetric == "bray") {
        pseq_transformed <- pseq %>%
          microbiome::transform(normalizationMethod) %>%  # Normalize to relative abundance
          microbiome::aggregate_taxa(level = taxRank)%>%  # Aggregate at the desired rank
          phyloseq::distance(dist = "bray", binary = FALSE)
      } else if (distanceMetric == "aitchison") {
        pseq_transformed <- pseq %>%
          microbiome::transform("identity") %>%  # Normalize to relative abundance
          microbiome::aggregate_taxa(level = taxRank)%>%  # Aggregate at the desired rank
          microbiome::distance(method = "aitchison")
      } else {
  ...
  observeEvent(input$plotPCoA, {
    req(
      ...
      input$taxRank,
      input$groupingColumnBeta_plotting,
      input$shapeColumnBeta
    )
    pseq <- current_physeq()

    tryCatch({
      pseq_transformed <-
        get_transformed_pseq(pseq,
                             input$distanceMetric,
                             input$normalizationMethod,
                             input$taxRank)
      ordination_res <-
        ord_calc(pseq_transformed, method = "PCoA")
      plot <-
        ord_plot(
          ordination_res,
        ...
        ggplot2::stat_ellipse(
          ggplot2::aes_string(
            fill = input$groupingColumnBeta_plotting,
            color = input$groupingColumnBeta_plotting
          ),
          linetype = 1,
          segments = 10,
        ...
        ggside::geom_xsideboxplot(
          ggplot2::aes_string(
            fill = input$groupingColumnBeta_plotting,
            y = input$groupingColumnBeta_plotting
          ),
          orientation = "y"
        ) +
        ggside::geom_ysideboxplot(
          ggplot2::aes_string(
            fill = input$groupingColumnBeta_plotting,
            x = input$groupingColumnBeta_plotting
          ),
          orientation = "x"
        ) +
    ...
    error = function(e) {
      output$betaDiversityNote <- renderText({
        e$message
      })
  ...
  observeEvent(input$plotPCA, {
    ...
    tryCatch({
      ordination_res <- pseq %>%
        microbiome::transform(input$normalizationMethod) %>%  # Normalize (e.g., relative abundance)
        microbiome::aggregate_taxa(level = input$taxRank)%>%  # Aggregate at desired rank
        ord_calc(method = "PCA")
      plot <-
        ord_plot(
          ordination_res,
  ...
  observeEvent(input$plotNMDS, {
    ...
    tryCatch({
      ...
      ordination_res <-
        ord_calc(pseq_transformed, method = "NMDS")

      # Print the stress value
      if ("stress" %in% names(ordination_res)) {
        # Check if the result has a stress attribute
        print(ordination_res$stress)
      } else if (inherits(ordination_res, "metaMDS")) {
        # Handle specific NMDS object types
        print(ordination_res$stress)
      } else {
        print(
          "Unable to retrieve stress value. Object does not contain a recognizable stress attribute."
        )
      }
      ...
      plot <-
        plot + ggplot2::scale_shape_manual(values = c(0:6, 15:20)) # Extend shapes if >6 groups
  ...
  observeEvent(input$calculatePERMANOVA, {
    req(
      current_physeq(),
      input$groupingColumnBeta_statistics,
      input$distanceMetric,
      input$taxRank
    )
    ...
    tryCatch({
      ...
      pseq_relabund <-
        aggregate_taxa(pseq_relabund, input$taxRank)
      ...
      meta <-
        data.frame(phyloseq::sample_data(pseq)) # Metadata as data frame

      # Validate grouping column exists in metadata
      validate(
        need(
          input$groupingColumnBeta_statistics %in% colnames(meta),
      ...
      formula <-
        as.formula(paste("otu_table ~", input$groupingColumnBeta_statistics))
  ...
  observeEvent(input$calculateDispersionTest, {
    ...
    validate(
      need(
        input$groupingColumnBeta_statistics %in% colnames(phyloseq::sample_data(pseq)),
        "Selected grouping column not found."
      )
    )
    ...
    tryCatch({
      ...
      dist <-
        vegan::vegdist(otu_table, method = input$distanceMetric)
      grouping_var <-
        phyloseq::sample_data(pseq_relabund)[[input$groupingColumnBeta_statistics]]
  ...
  observeEvent(input$show_beta_explanation, {
    showModal(
      modalDialog(
        ...
        HTML(
          "<p><strong>Beta Diversity</strong> is a measure of the differences between microbial communities from different environments or samples.</p>
  ...
  generateDriverPlot <- function(bestmodel, physeq, k) {
    ...
    d_tax_merge <-
      dplyr::left_join(d, tax_df %>% tibble::rownames_to_column("ASV"), by = "ASV")
    d <-
      d_tax_merge %>% dplyr::mutate(Taxa = paste0(Genus, "_", Species, " (", ASV, ")"))
    # d$Taxa <- with(tax_df[d$ASV, ], paste0(Genus, "_", Species, " (", d$ASV, ")"))


    # Replace NA values with "Unknown"
    d$Taxa[is.na(d$Taxa)] <-
      paste0("Unknown (", d$ASV[is.na(d$Taxa)], ")")
  ...
  observe({
    ...
    updateSelectInput(session,
                      "taxRankDMM",
                      choices = c(rank_names(pseq), "Genus", "Species"))
  })
  ...
  observeEvent(input$runDMM, {
    ...
    taxa <-
      microbiome::core_members(
        pseq.comp,
        detection = input$detectionThreshold / 100,
        prevalence = input$prevalenceThreshold / 100
      )
    ...
    if (nrow(count) > 0) {
      ...
      sample_assignments <-
        apply(DirichletMultinomial::mixture(best), 1, which.max)
      ...
      metadata <-
        as(phyloseq::sample_data(current_physeq()), "data.frame")
      ...
      updated_pseq <-
        phyloseq(
          otu_table(current_physeq()),
          phyloseq::tax_table(current_physeq()),
          phyloseq::sample_data(metadata_filtered),
          phy_tree(current_physeq())
        )
      ...
      output$downloadSampleAssignments <- downloadHandler(
        ...
        content = function(file) {
          sample_assignments_df <-
            sample_assignments_reactive() # Access the stored data
      ...
      output$mixtureParams <- renderPrint({
        "No valid samples with non-zero counts."
      })
      output$sampleAssignments <- renderDT({
        DT::datatable(
          data.frame(Sample = character(0), Cluster = character(0)),
          options = list(pageLength = 10, autoWidth = TRUE)
        )
      })
      output$driverPlotsUI <- renderUI({
  ...
  observeEvent(input$show_community_typing_explanation, {
    showModal(
      modalDialog(
        ...
        HTML(
          "<p><strong>Community Typing using Dirichlet Multinomial Mixture (DMM)</strong> is an approach to categorize or 'type' microbial communities into distinct groups based on their composition.</p>
  ...
  impute_median <- function(pseq) {
    ...
    for (col in colnames(metadata)) {
      if (is.numeric(metadata[[col]])) {
        imputed_metadata[[col]][is.na(imputed_metadata[[col]])] <-
          median(imputed_metadata[[col]], na.rm = TRUE)
      }
    }
  ...
  observe({
    ...
    if (!is.null(phyloseq::tax_table(reactive_pseq()))) {
      ...
      showNotification("Tax table is missing in the phyloseq object, using ASV level.",
                       type = "warning")
    }
  ...
  observeEvent(input$plotRDA, {
    ...
    tryCatch({
      ...
      if (!is.null(input$conditions) &&
        ...
        formula <-
          update(formula, paste0(
            ". ~ . + Condition(",
            paste(input$conditions, collapse = " + "),
            ")"
          ))
      }
      ...
      CAP_distance <-
        vegan::capscale(
          formula = formula,
          data = metadata,
          distance = input$RDAdistanceMetric,
          sqrt.dist = TRUE
        )
      ...
      species_scores <-
        vegan::scores(CAP_distance, display = "species")
      site_scores <-
        vegan::scores(CAP_distance, display = "sites")
      ...
      rda_plot <- ggplot2::ggplot() +
        ...
        ggplot2::geom_text(
          data = species_df,
          ggplot2::aes(x = CAP1, y = CAP2, label = Taxa),
          size = 3,
          hjust = 1.1,
          vjust = 1.1,
          color = "darkred"
      ...
      inertia_table <- cbind(Category = rownames(inertia_table),
                             inertia_table)
      rownames(inertia_table) <-
        NULL # Remove row names for cleaner output
      ...
      site_scores <-
        as.data.frame(scores(CAP_distance, display = "sites"))
      species_scores <-
        as.data.frame(scores(CAP_distance, display = "species"))
      env_vectors <- scores(CAP_distance, display = "bp")
      anova_overall <- as.data.frame(anova(CAP_distance))
      anova_terms <-
        as.data.frame(anova(CAP_distance, by = "terms", permu = 200))
      anova_axes <-
        as.data.frame(anova(CAP_distance, by = "axis", perm.max = 500))

      # Render summary inertia table
      output$inertiaTable <- DT::renderDT({
        DT::datatable(inertia_table,
                      options = list(pageLength = 5),
                      rownames = FALSE)
      })

      # Render other tables
      output$eigenTable <- DT::renderDT({
        DT::datatable(eigenvalues)
      })
      output$r2Table <- DT::renderDT({
        DT::datatable(data.frame(Adjusted_R2 = adjusted_r2))
      })
      output$coeffTable <- DT::renderDT({
        DT::datatable(regression_coefficients)
      })
      output$siteScoresTable <- DT::renderDT({
        DT::datatable(site_scores)
      })
      output$speciesScoresTable <- DT::renderDT({
        DT::datatable(species_scores)
      })
      output$anovaOverallTable <- DT::renderDT({
        DT::datatable(anova_overall)
      })
      output$anovaTermsTable <- DT::renderDT({
        DT::datatable(anova_terms)
      })
      output$anovaAxesTable <- DT::renderDT({
        DT::datatable(anova_axes)
      })
      ...
      output$downloadInertia <- downloadHandler(
        filename = function() {
          paste0("cap_inertia_summary_", Sys.Date(), ".csv")
        },
        content = function(file) {
          write.csv(inertia_table, file, row.names = FALSE)
        }
      )
      ...
      output$downloadR2 <- downloadHandler(
        ...
        content = function(file) {
          write.csv(data.frame(Adjusted_R2 = adjusted_r2), file)
        }
      )
      output$downloadCoeff <- downloadHandler(
        ...
        content = function(file) {
          write.csv(regression_coefficients, file)
        }
      )
